...
export const GET = withRole(['teacher'], async (request: NextRequest, auth: AuthContext) => {
  if (!auth.schoolId) {
    return NextResponse.json({ success: false, error: 'Missing school scope' }, { status: 401 });
  }

  const date = request.nextUrl.searchParams.get('date') || undefined;
  const data = await lmsPhase2Service.getTeacherAttendance(auth.schoolId, auth.userId, date);
