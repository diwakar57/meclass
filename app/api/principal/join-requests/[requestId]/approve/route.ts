...
const log = createLogger('ApproveJoinRequestAPI');
...
export async function POST(
  ...
  try {
    ...
    const membership = await SchoolService.approveStudentJoinRequest(requestId, payload.userId);
    ...
    if (isDbDown && isDevAuthFallbackEnabled()) {
      return NextResponse.json(
        {
          success: false,
          error: 'Cannot approve requests while database is unavailable in development mode.',
        },
        { status: 503 }
      );
    }
