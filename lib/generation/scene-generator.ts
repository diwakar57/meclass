...
export async function generateFullScenes(
  ...
  callbacks?.onProgress?.({
    ...
    overallProgress: 66,
    stageProgress: 0,
    statusMessage: `Generating ${totalScenes} scene(s) in parallel...`,
    scenesGenerated: 0,
    totalScenes,
  });
    ...
    sceneOutlines.map(async (outline, index) => {
      try {
        ...
        callbacks?.onProgress?.({
          ...
          statusMessage: `Completed ${completedCount}/${totalScenes} scene(s)`,
  ...
  return [
    {
      ...
      text: 'Now let’s begin a project-based learning activity. Please choose your role, check the issue board, and start collaborating.',
    },
  ];
}
  ...
  return [
    {
      ...
      text: 'Now let’s do a short quiz to check your learning outcomes.',
    },
  ];
}
  ...
  return [
    {
      ...
      text: 'Now let’s explore this concept through interactive visualization. Try interacting with elements on the page and observe the changes.',
    },
  ];
}
